...
export function spawnGatherersMinimumTask(logger: Logger) {
  basicSpawn(logger, "Spawn1", [MOVE, CARRY], config.spawning.gatherers.minimum, "Gatherer");
}

export function spawnGatherersPreferredTask(logger: Logger) {
  basicSpawn(logger, "Spawn1", [MOVE, CARRY], config.spawning.gatherers.preferred, "Gatherer");
}
